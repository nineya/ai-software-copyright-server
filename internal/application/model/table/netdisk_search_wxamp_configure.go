...
type NetdiskSearchWxampConfigure struct {
	...
	SecureMode         enum.NetdiskSecureMode `json:"secureMode" xorm:"notnull comment('所处的安全模式')" label:"所处的安全模式"`
	IsAudit            bool                   `json:"isAudit" xorm:"-" label:"是否是审核模式"` // TODO 兼容旧版本的搜索小程序
	ExtendTitle        string                 `json:"extendTitle" xorm:"VARCHAR(50) notnull comment('推广引流标题')" binding:"lte=50" label:"推广引流标题"`
	ExtendImageUrl     string                 `json:"extendImageUrl" xorm:"VARCHAR(512) notnull comment('推广引流图片url')" binding:"lte=512" label:"推广引流图片url"`
	BannerAdId         string                 `json:"bannerAdId" xorm:"VARCHAR(50) notnull comment('小程序横幅广告位ID')" binding:"lte=50" label:"小程序横幅广告位ID"`
	RewardAdId         string                 `json:"rewardAdId" xorm:"VARCHAR(50) notnull comment('小程序激励广告位ID')" binding:"lte=50" label:"小程序激励广告位ID"`
	ItemAdId           string                 `json:"itemAdId" xorm:"VARCHAR(50) notnull comment('小程序列表项广告位ID')" binding:"lte=50" label:"小程序列表项广告位ID"`
	...
	WelfareConfig      []common.WelfareLabel  `json:"welfareConfig" xorm:"TEXT notnull comment('小程序羊毛配置')" label:"小程序羊毛配置"`
...
func (NetdiskSearchWxampConfigure) TableName() string {
	return "netdisk_search_wxamp_configure"
}

func (t *NetdiskSearchWxampConfigure) SetUserId(userId int64) {
	t.UserId = userId
}
